...
Result nearestPoints_BF(std::vector<Point> &vp) {
    ...
    for(int i = 0; i < vp.size()-1; i++){
        for(int j = i+1; j < vp.size(); j++){
            ...
            if(currDistance < res.dmin){
                res = Result(currDistance, p1, p2);
            }
        }
    }
    return res;
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    ...
    return nearestPoints_DC_recursion(vp, vq);
}

Result nearestPoints_DC_recursion(std::vector<Point> &vp, std::vector<Point>& vq){
    if (vp.size() == 2) {
        return Result(vp[0].distance(vp[1]), vp[0], vp[1]);
    }
    if (vp.size() == 1) return Result(1E6, vp[0], vp[0]);
    Result left, right;
    std::vector<Point>::const_iterator middle = vp.begin() + vp.size() / 2;
    std::vector<Point> leftp((std::vector<Point>::const_iterator) vp.begin(), middle);
    std::vector<Point> rightp(middle, (std::vector<Point>::const_iterator) vp.end());

    std::vector<Point> leftq;
    std::vector<Point> rightq;
    for (int i = 0; i < vq.size(); ++i) {
        if (vq[i].x >= rightp[0].x) rightq.push_back(vq[i]);
        else leftq.push_back(vq[i]);
    }

    left = nearestPoints_DC_recursion(leftp, leftq);
    right = nearestPoints_DC_recursion(rightp, rightq);

    double minDist = std::min(left.dmin, right.dmin);
    std::vector<Point> pointsInStrip;
    double dividingLine = (double) (vp[0].x + vp[vp.size()-1].x) / 2;
    int ind1, ind2;
    for (int i = 0; i < vq.size(); ++i)
        if (std::abs(vq[i].x - dividingLine) < minDist)
            pointsInStrip.push_back(vq[i]);

    for (int i = 0; i < pointsInStrip.size(); ++i) {
        for (int j = i + 1; j < pointsInStrip.size(); ++j) {
            if (std::abs(pointsInStrip[i].y - pointsInStrip[j].y) >= minDist)
                break;
            double dist = pointsInStrip[i].distance(pointsInStrip[j]);
            if (dist < minDist) {
                minDist = dist;
                ind1 = i;
                ind2 = j;
    ...
    if (minDist < std::min(left.dmin, right.dmin))
        return Result(minDist, pointsInStrip[ind1], pointsInStrip[ind2]);
    if (left.dmin < right.dmin) return left;
    return right;
}
...
Result nearestPoints_DC_MT(std::vector<Point> &vp) {
    ...
    std::vector<Point> vq = vp;
    sortByY(vq, 0, vq.size()-1);
    res = nearestPoints_DC_MT_recursion(vp, vq);
    return res;
}

Result nearestPoints_DC_MT_recursion(std::vector<Point> &vp, std::vector<Point>& vq){
    if (vp.size() == 2) {
        return Result(vp[0].distance(vp[1]), vp[0], vp[1]);
    }
    if (vp.size() == 1) return Result(1E6, vp[0], vp[0]);
    ...
    if(numThreads > 1){
        std::thread t([&](){
            left = nearestPoints_DC_MT_recursion(leftp, leftq);    // makes recursive call in another thread
        });
        numThreads /= 2;
        right = nearestPoints_DC_MT_recursion(rightp, rightq); // runs this recursion on this thread
        t.join();   // wait for the other thread to end
    }
    else{
        left = nearestPoints_DC_MT_recursion(leftp, leftq);
        right = nearestPoints_DC_MT_recursion(rightp, rightq);
    }

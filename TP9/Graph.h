...
void Graph<T>::testAndVisit(queue< Vertex<T>*> &q, Edge<T> *e, Vertex<T> *w, double residual) {
	...
	if (! w-> visited && residual > 0 && e->cost == 0) {    // added the cost condition
		w->visited = true;
		w->path = e;
		q.push(w);
	}
...
double Graph<T>::minCostFlow(T source, T sink, double flow) {
    double currFlow = 0;
    double currCost = 0;
    double currUnitCost = 0;

    int iter = 0;
    Vertex<T>* src = this->findVertex(source);
    Vertex<T>* dest = this->findVertex(sink);

    this->resetFlows();

    while((currFlow < flow)){

        if(iter == 0)       // calcular distancias minimas a todos os vertices
            this->bellmanFordShortestPath(src);
        else
        ...
        currUnitCost += dest->dist;

        for(Vertex<T>* v : this->vertexSet){    // Substituir os custos inicias
            for(Edge<T>* e : v->outgoing){
                Vertex<T>* u = e->orig;
                Vertex<T>* v = e->dest;
                e->cost = e->cost + u->dist - v->dist;
            }
        }

        do  {       // enquanto exisir um "augmentation path" com custo alterado = 0
            double f = min( this->findMinResidualAlongPath(src, dest), flow - currFlow);
            this->augmentFlowAlongPath(src, dest, f);

            currFlow += f;
            currCost += currUnitCost * f;   // currUnitCost is the cost to go from the src to destiny
        } while(currFlow < flow && this->findAugmentationPath(src, dest));
